...
from ml.features.schema import (
    ...
    NIGHTFIRE_MATCH_DISTANCE_M,
    WORLDPOP_DEFAULT_DENSITY,
)
...
class FIRMSFeatureExtractor:
    ...
    def extract(record: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "is_night": is_night,
            "frp": round(frp, 2),
            "frp_zscore": round(float(record.get("frp_zscore", 0.0)), 3),
...
class NightfireFeatureExtractor:
    """
    Extracts features from VIIRS Nightfire data (gas flare / persistent thermal sources).
    """

    @staticmethod
    def extract(nightfire: Any) -> Dict[str, Any]:
        if not nightfire or not isinstance(nightfire, dict):
            return {
                "nearest_nightfire_distance_m": DEFAULT_MAX_DISTANCE_M,
                "nightfire_detection_freq": 0.0,
                "is_nightfire_match": 0,
            }

        # Distance to nearest nightfire record (meters)
        dist_m = nightfire.get("nearest_distance_m")
        if dist_m is None:
            nearest_dist = DEFAULT_MAX_DISTANCE_M
        else:
            nearest_dist = float(dist_m)

        # Detection frequency (0–1)
        freq = float(nightfire.get("detection_frequency", 0.0))

        # Matched if within threshold
        is_match = 1 if nearest_dist <= NIGHTFIRE_MATCH_DISTANCE_M else 0

        return {
            "nearest_nightfire_distance_m": round(nearest_dist, 2),
            "nightfire_detection_freq": round(freq, 3),
            "is_nightfire_match": is_match,
        }


class PowerPlantFeatureExtractor:
    """
    Extracts features from WRI Global Power Plant Database (industrial hazard context).
    """

    @staticmethod
    def extract(powerplants: Any) -> Dict[str, Any]:
        if not powerplants or not isinstance(powerplants, dict):
            return {
                "nearest_powerplant_distance_m": DEFAULT_MAX_DISTANCE_M,
                "powerplant_capacity_mw": 0.0,
                "powerplant_count": 0,
            }

        # Nearest power plant distance
        dist_m = powerplants.get("nearest_distance_m")
        if dist_m is None:
            nearest_dist = DEFAULT_MAX_DISTANCE_M
        else:
            nearest_dist = float(dist_m)

        # Capacity of nearest plant (MW)
        capacity = float(powerplants.get("capacity_mw", 0.0))

        # Number of power plants within search radius
        count = int(powerplants.get("count", 0))

        return {
            "nearest_powerplant_distance_m": round(nearest_dist, 2),
            "powerplant_capacity_mw": round(capacity, 1),
            "powerplant_count": count,
        }


class WorldPopFeatureExtractor:
    """
    Extracts population density from WorldPop raster.
    """

    @staticmethod
    def extract(worldpop: Any) -> Dict[str, Any]:
        if worldpop is None:
            return {
                "population_density": WORLDPOP_DEFAULT_DENSITY,
            }
        # worldpop is expected to be a dict like {"density": 123.4} or a number
        if isinstance(worldpop, dict):
            density = float(worldpop.get("density", WORLDPOP_DEFAULT_DENSITY))
        else:
            try:
                density = float(worldpop)
            except (TypeError, ValueError):
                density = WORLDPOP_DEFAULT_DENSITY

        return {
            "population_density": round(density, 2),
        }

class ClusterDensityFeatureExtractor:
    @staticmethod
    def extract(cluster_info: Any) -> Dict[str, Any]:
        if not cluster_info or not isinstance(cluster_info, dict):
            return {"cluster_density": 0}
        return {"cluster_density": int(cluster_info.get("count", 0))}